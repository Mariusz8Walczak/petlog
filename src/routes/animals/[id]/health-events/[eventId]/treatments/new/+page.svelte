...
<svelte:head><title>Nowe leczenie — PetLog</title></svelte:head>

<h1 class="mb-4 text-xl font-semibold">Nowe leczenie — {data.event.symptom}</h1>
...
<form method="POST" use:enhance class="flex max-w-md flex-col gap-3">
	...
	<label class="flex flex-col gap-1 text-sm">
		Dawka (opcjonalnie)
		<input name="dosage" class="rounded border border-gray-300 px-3 py-2" />
	</label>
	<label class="flex flex-col gap-1 text-sm">
		Data rozpoczęcia
		<input type="date" name="startDate" required class="rounded border border-gray-300 px-3 py-2" />
	</label>
	<label class="flex flex-col gap-1 text-sm">
		Data zakończenia (opcjonalnie)
		<input type="date" name="endDate" class="rounded border border-gray-300 px-3 py-2" />
	</label>
	<label class="flex flex-col gap-1 text-sm">
		Skutek
		<select name="outcome" class="rounded border border-gray-300 px-3 py-2">
			{#each data.outcomes as o (o)}
				<option value={o}>{outcomeLabel[o] ?? o}</option>
			{/each}
		</select>
	</label>
	<label class="flex flex-col gap-1 text-sm">
